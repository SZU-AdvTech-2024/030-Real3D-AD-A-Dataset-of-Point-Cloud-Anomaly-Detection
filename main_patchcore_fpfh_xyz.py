...
import patchcore.patchcore_xyz
...
def run(
    ...
    for dataset_count, dataset_name in enumerate(real_3d_classes):
        ...
        with device_context:
            ...
            nn_method = patchcore.common_pointCore.FaissNN(faiss_on_gpu, faiss_num_workers)

            PatchCore = patchcore.patchcore_xyz.PatchCore(device)
            PatchCore.load(
                backbone=None,
            ...
            torch.cuda.empty_cache()
            memory_feature = PatchCore.fit_with_limit_size_(train_loader, memory_size)
            aggregator_fpfh = {"scores": [], "segmentations": []}
            start_time = time.time()
            scores_fpfh, segmentations_fpfh, labels_gt, masks_gt = PatchCore.predict_fpfh_upxyz(
                test_loader
            )
            ...
            with open('target.txt', "a", encoding="utf-8") as file:
                file.write('Task:{}, image_auc:{}, pixel_auc:{}, image_ap:{}, pixel_ap:{}, time_cost:{}\n\n'.format(
                    dataset_name, auroc, full_pixel_auroc, img_ap, pixel_ap, time_cost))
            number_classes += 1
            image_auc_sum += auroc
            pixel_auc_sum += full_pixel_auroc
            image_ap_sum += img_ap
            pixel_ap_sum += pixel_ap
    ...
    # )
    with open('target.txt', "a", encoding="utf-8") as file:
                file.write(f'sum:{number_classes},image_auc:{round(image_auc_sum / number_classes, 4)}, pixel_auc:{round(pixel_auc_sum/number_classes,4)}, image_ap:{round(image_ap_sum/number_classes,4)}, pixel_ap:{round(pixel_ap_sum/number_classes,4)}, time_cost:{round(time_cost_sum/number_classes,4)}\n\n')
