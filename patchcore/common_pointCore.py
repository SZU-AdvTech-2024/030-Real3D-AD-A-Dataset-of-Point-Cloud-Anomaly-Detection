...
class FaissNN(object):
    ...
    def fit(self, features: np.ndarray) -> None:
        ...
        if self.search_index:
            self.reset_index()
        self.search_index = self._create_index(3)
        self._train(self.search_index, features)
        xyz_features = features[:, -3:]
        xyz_features = np.ascontiguousarray(xyz_features)
        self.features = features[:, :-3]
        # print(f"self.features:{self.features.shape}")
        # print(f"self.features:{self.features}")
        
        self.search_index.add(xyz_features)
    ...
    def run(
        ...
        if index_features is None:
            # print(f"query_features:{query_features.shape}") # query_features.shape:(154715, 1155)
            # print(n_nearest_neighbours) # 1
            xyz_features = query_features[:, -3:]
            xyz_features = np.ascontiguousarray(xyz_features)
            query_distances, query_nns = self.search_index.search(xyz_features, n_nearest_neighbours)
            # self.features[query_nns]
            # query_features[:, :1152]
            # print(f"query_distances:{query_distances.shape}")
            # print(f"query_distances:{query_distances}")
            # print(f"query_nns:{query_nns.shape}")
            # print(f"query_features[:, :1152]:{query_features[:, :1152].shape}")
            # print(f"self.features[query_nns]:{self.features[query_nns].shape}")
            
#             nearest_neighbor_features = self.features[query_nns]  # 形状: (16384, 3, 1152)

# # 将最近邻特征的维度从 (16384, 3, 1152) 重塑为 (16384 * 3, 1152)
#             nearest_neighbor_features_reshaped = nearest_neighbor_features.reshape(-1, nearest_neighbor_features.shape[-1])
#             print(f"nearest_neighbor_features_reshaped:{nearest_neighbor_features_reshaped.shape}")
#             distances = cdist(query_features[:, :1152], nearest_neighbor_features_reshaped, metric='euclidean')
#             print(f"distances:{distances.shape}")
            query_pmae_features = query_features[:, :-3]# 形状: (154715, 1152)
            query_xyz_features = query_features[:, -3:]# 形状: (154715, 3)
            # print(f"query_pmae_features:{query_pmae_features.shape}")
# 提取对应最近邻的特征，形状为 (16384, 3, 1152)
            nearest_neighbor_features = self.features[query_nns]  # 形状: (154715, 3, 1152)
            # print(f"nearest_neighbor_features:{nearest_neighbor_features.shape}")
            
# 使用 cdist 计算每个查询点与其对应的 3 个最近邻之间的欧几里得距离
# 对于每个查询点，我们计算它与 3 个邻居之间的距离

            distances_pmae = np.zeros((query_pmae_features.shape[0], nearest_neighbor_features.shape[1]))# 形状: (154715, 3)
            # print(f"distances:{distances.shape}")
            distances_xyz = query_distances
            for i in range(query_pmae_features.shape[0]):
    # 计算单个查询点与它的 3 个最近邻的欧几里得距离
                distances_pmae[i] = cdist(query_pmae_features[i:i+1], nearest_neighbor_features[i],metric='euclidean').flatten()
            # print(f"distances:{distances.shape}")
            
            return distances_pmae, query_distances, query_nns
...
class NearestNeighbourScorer(object):
    ...
    def predict(
        ...
        query_pmae_distances, query_xyz_distances, query_nns = self.imagelevel_nn(query_features)
        
        anomaly_pmae_scores = np.min(query_pmae_distances, axis=-1)
        anomaly_xyz_scores = np.min(query_xyz_distances, axis=-1)
        # anomaly_xyz_scores = np.min(query_xyz_distances, axis=-1)
        
        return anomaly_pmae_scores, query_pmae_distances, query_nns
