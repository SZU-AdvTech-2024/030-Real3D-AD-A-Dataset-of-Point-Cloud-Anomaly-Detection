...
class FaissNN(object):
    ...
    def fit(self, features: np.ndarray) -> None:
        ...
        if self.search_index:
            self.reset_index()
        # print(f"记忆库特征维度:{features.shape[-1]}")
    ...
    def run(
        ...
        # print(f"index_features:{index_features}")
        if index_features is None:
            a, b = self.search_index.search(query_features, n_nearest_neighbours)
            # print(f"a:{type(a)}")
            # print(f"b:{type(b)}")
...
class NearestNeighbourScorer(object):
    ...
    def predict(
        ...
        # print(f"query_features:{type(query_features)}")
